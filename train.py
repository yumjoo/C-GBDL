...
def parse_args():
    ...
    parser.add_argument('-b', '--batch_size', default=2, type=int,
                        metavar='N',
                        help='mini-batch size. Note that this is the batch size of patients rather than CT slices')
    ...
    # dataset
    parser.add_argument('--dataset', default='Kits19',
                        help='dataset name')
        ...
        if total_num == 0:
            ...
            for i in range(4):
                if fixed_input == None:
                    ...
                    fixed_input = torch.cat([fixed_input, torch.from_numpy(info[5][i]).unsqueeze(0).unsqueeze(0)],
                                            dim=1)
                    fixed_target = torch.cat([fixed_target, torch.from_numpy(info[6][i]).unsqueeze(0).unsqueeze(0)],
                                             dim=1)
            fixed_inputs = fixed_input
            fixed_targets = fixed_target
            input_paths = info[3]
        else:
            ...
            for i in range(4):
                if fixed_input == None:
                    ...
                    fixed_target = torch.cat([fixed_target, torch.from_numpy(info[6][i]).unsqueeze(0).unsqueeze(0)],
                                             dim=1)
            fixed_inputs = torch.cat([fixed_inputs, fixed_input], dim=0)
            fixed_targets = torch.cat([fixed_targets, fixed_target], dim=0)
            input_paths = np.dstack((input_paths, info[3]))
        num_per_patient.append(info[2])
        total_num += 1

    return input.to(torch.float32), target, num_per_patient, input_paths, info[4], fixed_inputs.to(torch.float32), fixed_targets
...
def train(config, train_loader, model, model_seg, criterion, optimizer, epoch):
    ...
    for input, target, num_per_p, paths, patient, fixed_imgs, fixed_mask_one_hots in train_loader:
        ...
        fixed_mask_one_hots = fixed_mask_one_hots.cuda(non_blocking=True)

        with torch.no_grad():
            input_var = torch.autograd.Variable(input)
            target_var = torch.autograd.Variable(target)
            fixed_input_var = torch.autograd.Variable(fixed_imgs)
            fixed_target_var = torch.autograd.Variable(fixed_mask_one_hots)
        ...
        fixed_img_ = torch.transpose(fixed_input_var, 2, 3)
        fixed_target_ = torch.transpose(fixed_target_var, 2, 3)
        ...
        if epoch > int(config['epochs'] / 2):
            with torch.no_grad():
                x_ori, output, _, _ = model(input_, fixed_img_, fixed_target_, 2, 1, config['N_Ref'])
            ...
            if target_real is None:
                ...
                crit_seg = criterion[0](output_labeled, target_real, num_classes=config['num_classes']) + criterion[0](
                    output_unlabeled, target_pseudo, num_classes=config['num_classes'])
                crit_seg_dice = criterion[1](output_labeled, target_real) + criterion[1](output_unlabeled,
                                                                                         target_pseudo)
                iou = dice_coef(torch.softmax(output_unlabeled, dim=1), target_pseudo) + dice_coef(
                    torch.softmax(output_labeled, dim=1), target_real)

        else:
            x_ori, output, mean, covar = model(input_, fixed_img_, fixed_target_, 1, 1, 1)
            ...
            for i in range(output.size(0)):
                ...
                if name_p not in labeled_patient:
                    ...
                    crit += criterion[0](output[i, :, :, :, :].unsqueeze(0), target_[i, :, :, :, :].unsqueeze(0),
                                         num_classes=config['num_classes'])
                    crit_dice += criterion[1](output[i, :, :, :, :].unsqueeze(0), target_[i, :, :, :, :].unsqueeze(0))
                    iou += dice_coef(torch.softmax(output[i, :, :, :, :].unsqueeze(0), dim=1),
                                     target_[i, :, :, :, :].unsqueeze(0))
                    batch_ += 1

        if epoch <= int(config['epochs'] / 2):
            if batch_ > 0:
                loss = crit / batch_ + 2 * crit_dice / batch_ + recon / input_.size(0) + 0.005 * KLD/input_.size(0)
            else:
                loss = recon / input_.size(0) + 0.005 * KLD/input_.size(0)
        else:
...
def validate(config, val_loader, model, model_seg, criterion, epoch):
    ...
    with torch.no_grad():
        ...
        if epoch <= int(config['epochs'] / 2):
            for input, target, _, _, _, fixed_img, fixed_mask_one_hot in val_loader:
                input = input.cuda(non_blocking=True)
                target = target.cuda(non_blocking=True)
                fixed_img = fixed_img.cuda(non_blocking=True)
                fixed_target = fixed_mask_one_hot.cuda(non_blocking=True)

                input_var = torch.autograd.Variable(input)
                target_var = torch.autograd.Variable(target)
                fixed_img_var = torch.autograd.Variable(fixed_img)
                fixed_target_var = torch.autograd.Variable(fixed_target)

                input_ = torch.transpose(input_var, 1, 2)
                target_ = torch.transpose(target_var, 1, 2)
                fixed_img_ = torch.transpose(fixed_img_var, 2, 3)
                fixed_target_ = torch.transpose(fixed_target_var, 2, 3)

                x_ori, output, _, _ = model(input_, fixed_img_, fixed_target_, 1, 1, config['N_Ref'])

                loss = criterion[0](output, target_, num_classes=config['num_classes']) / output.size(0) + criterion[1](
                    output, target_)
            ...
            for input, target, _, _, _, _, _ in val_loader:
                T = 5
                out_seg = None
                out_seg_ = None
...
def main():
    ...
    model_seg = torch.nn.DataParallel(model_seg, device_ids=[0, 1]).cuda()
    ...
    for epoch in range(config['start_epoch'], config['epochs']):
        ...
        if epoch > int(config['epochs'] / 2):
            ...
            save_checkpoint({
                ...
                'best_iou': best_iou,
                'optimizer': optimizer.state_dict(),
            }, filename='%s/%s/model.pth' % (config['model_save_dir'], config['name']))
