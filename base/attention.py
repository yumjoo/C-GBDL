...
from base.conv_4d import Conv4d


class CorrAttention(Module):

    def __init__(self, batch, num_heads, depth, height, weight, feat_dim):
        ...
        self.height = height
        self.weight = weight
        self.feat_size = height * weight
        self.depth = depth

        self.conv_4d = Conv4d(num_heads, num_heads, (3, 3, 3, 3), (1, 1, 1, 1), (1, 1, 1, 1), self.feat_size, feat_dim)
        self.gn = nn.GroupNorm(num_heads//2, num_heads)
        self.relu = nn.ReLU(inplace=True)
        
        # self.conv_4d_ = Conv4d(num_heads, num_heads, (3, 3, 3, 3), (1, 1, 1, 1), (1, 1, 1, 1), self.feat_size, feat_dim)
        # self.gn_ = nn.GroupNorm(num_heads//2, num_heads)
        # self.relu_ = nn.ReLU(inplace=True)

    def forward(self, corr_map):
        s = corr_map.shape
        corr_map = corr_map.reshape(self.batch, self.num_heads, self.depth, self.height, self.weight, self.height, self.weight)
        corr_map = self.relu(self.gn(self.conv_4d(corr_map)))
        # corr_map = self.relu_(self.gn_(self.conv_4d_(corr_map)))
        
        corr_map = corr_map.reshape(s)
...
class AiAModule(Module):
    ...
    def __init__(self, embed_dim, batch, num_heads, depth, height, weight, feat_dim, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
                 kdim=None, vdim=None, batch_first=False, device=None, dtype=None, use_AiA=True) -> None:
        ...
        if self.use_AiA:
            self.inner_attn = CorrAttention(batch, num_heads, depth, height, weight, feat_dim)
    ...
    def aia_scaled_dot_product_attention(
        ...
        if self.use_AiA:
            corr_map = attn
            corr_map = self.inner_attn(corr_map)
            attn = attn + corr_map
